...
function shakeScreen(){const g=document.getElementById('game');if(!g)return;g.classList.remove('shake-screen');void g.offsetWidth;g.classList.add('shake-screen');triggerVibe('heavy');playSound('clash');}
...
function connectSocket(){
  ...
  socket.on('player_data',d=>{
    const w=document.getElementById('stat-wins'), l=document.getElementById('stat-losses');
    if(w)w.textContent=d.wins||0; if(l)l.textContent=d.losses||0;
  ...
  socket.on('rematch_pending',({count})=>{const g=document.getElementById('go-pending');if(g)g.textContent=count>=1?'Ждём…':'';});
  socket.on('opponent_left',()=>{showToast('Соперник ушёл');setTimeout(goToMenu,1800);});
  socket.on('player_surrendered',({name})=>{
    const sText = document.getElementById('surrender-text');
    const sNot = document.getElementById('surrender-notice');
    if(sText) sText.textContent=`💀 ${name} позорно слился!`;
    if(sNot) { sNot.classList.add('show'); setTimeout(()=>sNot.classList.remove('show'),3000); }
  });
  socket.on('error_msg',msg=>showToast(msg));
}

// ═══ МЕНЮ И ЛОББИ ═══
bindClick('btn-play', ()=>showScreen('lobby-browser'));
bindClick('btn-backs', ()=>{renderBacks();showScreen('backs');});
bindClick('lb-back', ()=>showScreen('menu'));
bindClick('lc-back', ()=>showScreen('lobby-browser'));
bindClick('lc-create-btn', ()=>{socket.emit('create_lobby',{user:ME,maxPlayers:createSlotCount});showScreen('lobby');});
bindClick('btn-create-lobby', ()=>{renderCreateLobby();showScreen('lobby-create');});
bindClick('backs-back', ()=>showScreen('menu'));

function renderLobbyList(list){const el=document.getElementById('lb-list');if(!el)return;el.innerHTML='';if(!list||!list.length){el.innerHTML='<div class="lb-empty">Нет открытых лобби.</div>';return;}list.forEach(l=>{const card=document.createElement('div');card.className='lb-card';card.innerHTML=`<div class="lb-card-info"><div class="lb-card-name">${esc(l.creatorName)}</div><div class="lb-card-count">${l.playerCount}/${l.maxPlayers}</div></div>`;const btn=document.createElement('button');btn.className='btn btn-secondary';btn.innerHTML='<span>Войти</span>';btn.addEventListener('click',()=>{socket.emit('join_lobby',{lobbyId:l.id,user:ME});showScreen('lobby');});card.appendChild(btn);el.appendChild(card);});}
let createSlotCount=2;function renderCreateLobby(){createSlotCount=2;updateCreateSlots();}
function updateCreateSlots(){const el=document.getElementById('lc-slots');if(!el)return;el.innerHTML='<div class="lc-slot lc-slot--you">Вы</div>';for(let i=1;i<createSlotCount;i++){const s=document.createElement('div');s.className='lc-slot lc-slot--added';s.innerHTML=`<span>Слот ${i+1}</span><div class="lc-remove">✕</div>`;el.appendChild(s);}if(createSlotCount<4){const add=document.createElement('div');add.className='lc-slot lc-slot--empty';add.textContent='＋';add.addEventListener('click',()=>{if(createSlotCount<4){createSlotCount++;updateCreateSlots();}});el.appendChild(add);}el.querySelectorAll('.lc-remove').forEach(btn=>{btn.addEventListener('click',e=>{e.stopPropagation();if(createSlotCount>2){createSlotCount--;updateCreateSlots();}});});}

function onLobbyJoined(lobby){
  currentLobby=lobby;
  const rId = document.getElementById('lobby-room-id'); if(rId) rId.textContent=lobby.id;
  const c=document.getElementById('lobby-players'); if(!c)return; c.innerHTML='';
  for(let i=0;i<lobby.maxPlayers;i++){const p=lobby.players[i],el=document.createElement('div');el.className='lobby-player';if(p)el.innerHTML=`<div class="lobby-player-avatar"${p.avatar?` style="background-image:url('${p.avatar}')"`:''}>${p.isBot?'🤖':''}</div><div class="lobby-player-name">${esc(p.name)}</div>`;else{el.style.opacity='.4';el.innerHTML='<div class="lobby-player-avatar" style="border-style:dashed">?</div>';}c.appendChild(el);}
  const actions=document.getElementById('lobby-actions'),wait=document.getElementById('lobby-wait'),amCreator=lobby.creatorId===ME.id;
  if(actions && wait) {
    if(amCreator){actions.style.display='flex';wait.style.display='none'; const sg = document.getElementById('btn-start-game'); if(sg) sg.style.display=lobby.playerCount>=2?'flex':'none';}
    else{actions.style.display='none';wait.style.display='flex';}
  }
}

bindClick('btn-start-game', ()=>socket.emit('start_game'));
bindClick('btn-add-bot', ()=>socket.emit('add_bot'));
bindClick('btn-leave-lobby', ()=>window.location.reload());
bindClick('btn-leave-lobby2', ()=>window.location.reload());
bindClick('btn-invite-friend', ()=>{if(!currentLobby)return;const link=`${BOT_LINK}?startapp=${currentLobby.id}`;if(tg?.openTelegramLink)tg.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(link)}&text=${encodeURIComponent('ДУЭЛЬ!')}`);});

function renderBacks(){const grid=document.getElementById('backs-grid');if(!grid)return;grid.innerHTML='';AVAILABLE_BACKS.forEach(b=>{const opt=document.createElement('div');opt.className='back-option'+(b.id===mySelectedBack?' selected':'');opt.innerHTML=`<img src="assets/backs/${b.id}.png" onerror="this.src='assets/cards/back.png'"/><div class="back-option-name">${b.name}</div>`;opt.addEventListener('click',()=>{socket.emit('set_back',{userId:ME.id,backName:b.id});mySelectedBack=b.id;renderBacks();});grid.appendChild(opt);});}
...
const TUTORIAL_STEPS=[
  ...
  {text:'На своём ходу ты берёшь 1 карту (автоматически). Теперь у тебя 2 карты — выбери одну и сыграй её.', cards:'hand', highlight:'arrow', show:[{v:1, glow:true},{v:6}]},
...
function renderTutStep(){
  ...
  const tText = document.getElementById('tut-text'); if(tText) tText.textContent=step.text;
  const area=document.getElementById('tut-game-area'); if(!area)return; area.innerHTML='';
  ...
  if(step.show && step.show.length>0){
    ...
    step.show.forEach(c=>{
      ...
      const card=makeCard(c.v==='back'?null:{value:c.v}, c.v!=='back', 'card--big', mySelectedBack);
...
bindClick('tut-next', ()=>{tutStep++; renderTutStep(); playSound('click');});
bindClick('tut-skip', ()=>showScreen('menu'));
...
function resetGameState(){lastState=null;stateQueue=[];busyAnimating=false;prevOppDiscardLen=0;activeRevealCard=null;const vl=document.getElementById('vfx-layer');if(vl)vl.innerHTML='';}
...
// ─── АНИМАЦИИ И VFX ───
function animateOppRevealPart1(playedCard,backName,cb){const layer=document.getElementById('vfx-layer');if(!layer)return cb();layer.innerHTML='';const ex=document.createElement('div');ex.className='card card--big';ex.innerHTML=`<div class="card-back"><img src="assets/backs/${backName}.png" onerror="this.src='assets/cards/back.png'"></div><div class="card-face"><img src="assets/cards/${playedCard.value}.png"></div>`;ex.style.cssText='position:absolute;top:-10%;transform:scale(0.5);transition:all 0.8s cubic-bezier(0.25,1,0.5,1)';layer.appendChild(ex);void ex.offsetWidth;ex.style.top='50%';ex.style.transform='translateY(-50%) scale(2.2)';playSound('card');setTimeout(()=>{ex.classList.add('face-up');playSound('card');activeRevealCard=ex;setTimeout(cb,600);},800);}
...
function handleVFX(data,callback=()=>{const isDirect=!busyAnimating;if(isDirect)busyAnimating=true;const layer=document.getElementById('vfx-layer');if(!layer)return callback();let dur=2000;
...
function renderState(s){
  const mN = document.getElementById('me-name'), mS = document.getElementById('me-status'), mA = document.getElementById('me-avatar'), dC = document.getElementById('deck-count-badge');
  if(mN) mN.textContent=s.me?.name||'Вы';
  if(mS) mS.textContent=s.isMyTurn?'твой ход':(s.me?.protected?'под защитой':'');
  if(s.me?.avatar && mA) mA.style.backgroundImage=`url('${s.me.avatar}')`;
  ...
  if(dC) dC.textContent=s.deckCount; 
...
function renderOpponents(s){
  const zone=document.getElementById('g-opponents'); if(!zone)return; zone.innerHTML='';
...
function renderMyCards(s){
  const zone=document.getElementById('my-card-zone'); if(!zone)return; zone.innerHTML='';
...
function updateLogStrip(log){const l=(log||[]).filter(l=>l&&!l.startsWith('—')); 
const l1=document.getElementById('log-line-1'), l2=document.getElementById('log-line-2');
if(l1)l1.textContent=l[l.length-1]||'—'; if(l2)l2.textContent=l[l.length-2]||'';}

bindClick('log-strip', ()=>{if(!lastState)return; const list=document.getElementById('lo-list'); if(!list)return; list.innerHTML=''; [...(lastState.log||[])].reverse().forEach(line=>{const d=document.createElement('div'); d.className='lo-entry'+(line.startsWith('—')?' lo-entry--round':''); d.textContent=line; list.appendChild(d);}); const lo=document.getElementById('log-overlay'); if(lo) lo.classList.add('show');});

function renderDiscard(id,cards){const el=document.getElementById(id); if(!el)return; el.innerHTML=''; cards.slice(-7).forEach(c=>{const card=makeCard(c,true,'card--sm', mySelectedBack); card.onclick=()=>openZoom(c); el.appendChild(card);});}
function renderExcluded(cards){const el=document.getElementById('excluded-cards'); if(!el)return; el.innerHTML=''; cards.forEach(c=>{const card=makeCard(c,true,'card--exc', mySelectedBack); card.onclick=()=>openZoom(c); el.appendChild(card);});}
...
function openGuessModal(card,targetUserId){
  const el=document.getElementById('action-modal'),g=document.getElementById('action-options'); if(!el||!g)return; g.innerHTML='';
  for(let v=0;v<=9;v++){
...
function openFederalModal(card,s){
  const el=document.getElementById('target-modal'),o=document.getElementById('target-options'); if(!el||!o)return; o.innerHTML='';
...
function showChancellor(cards){
  const el=document.getElementById('chancellor-modal'),w=document.getElementById('chancellor-cards'); if(!el||!w)return; w.innerHTML='';
...
function openZoom(card){if(!card)return; const def=CARDS[card.value],w=document.getElementById('cz-card-img'); if(!w)return; w.innerHTML=`<img src="assets/cards/${card.value}.png">`; document.getElementById('cz-name').textContent=def.name; document.getElementById('cz-desc').textContent=def.desc; document.getElementById('cz-seen').textContent=`Видено: ${(lastState?.me?.seenCounts||{})[card.value]||0}/${def.total}`; document.getElementById('card-zoom').classList.add('show');}

function showPeek(data){const w=document.getElementById('peek-card'); if(!w)return; w.innerHTML=''; if(data.card)w.appendChild(makeCard(data.card,true,'card--big', mySelectedBack)); document.getElementById('peek-card-name').textContent=data.cardName; document.getElementById('peek-overlay').classList.add('show');}

function showRoundOver(ro){const ov=document.getElementById('round-over');if(!ov)return; const iW=ro.winnerId===ME.id; document.getElementById('ro-title').textContent=iW?'✦ ПОБЕДА!':'✗ ПОРАЖЕНИЕ'; document.getElementById('ro-reason').textContent=ro.reason; if(!iW)shakeScreen(); else playSound('success'); ov.classList.add('show'); setTimeout(()=>ov.classList.remove('show'),4000);}
function showGameOver(go){const ov=document.getElementById('game-over');if(!ov)return; document.getElementById('go-reason').textContent=go.reason; document.getElementById('go-tokens-row').innerHTML=go.scores.map(s=>`<span>${esc(s.name)}: <strong>${s.tokens}</strong></span>`).join(' | '); ov.classList.add('show');}

bindClick('btn-surrender', ()=>{if(confirm('Сдаться?'))socket.emit('surrender');});
bindClick('btn-rematch', ()=>socket.emit('rematch'));
bindClick('btn-to-menu', goToMenu);
...
// КРЕСТИКИ
bindClick('cz-close', ()=>{const e=document.getElementById('card-zoom');if(e)e.classList.remove('show');});
bindClick('lo-close', ()=>{const e=document.getElementById('log-overlay');if(e)e.classList.remove('show');});
bindClick('do-close', ()=>{const e=document.getElementById('deck-overlay');if(e)e.classList.remove('show');});
bindClick('action-cancel', ()=>{const e=document.getElementById('action-modal');if(e)e.classList.remove('show');});
bindClick('target-cancel', ()=>{const e=document.getElementById('target-modal');if(e)e.classList.remove('show');});
bindClick('peek-close', ()=>{const e=document.getElementById('peek-overlay');if(e)e.classList.remove('show');});

bindClick('deck-btn', ()=>{
  const g=document.getElementById('do-grid'); if(!g)return; g.innerHTML=''; 
  for(let v=0;v<=9;v++){
    ...
    r.onclick=()=>{
      const bx=document.getElementById('do-detail-box'), nm=document.getElementById('do-detail-name'), ds=document.getElementById('do-detail-desc');
      if(bx)bx.style.display='block'; if(nm)nm.textContent=d.name; if(ds)ds.textContent=d.desc;
    }; 
    g.appendChild(r);
  } 
  const ov=document.getElementById('deck-overlay'); if(ov)ov.classList.add('show');
});
