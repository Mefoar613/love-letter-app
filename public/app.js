// =====================================================================
// Тёмная Дуэль — Frontend v3.5 (Полный фикс)
// =====================================================================
...
function processState(s) {
  ...
  if (oppPlayed) {
    ...
    animateOppRevealAndPlay(playedCard, () => {
      ...
      renderState(s, false, true);
      busyAnimating = false;
      if (stateQueue.length > 0) { const next = stateQueue.shift(); setTimeout(() => processState(next), 50); }
    });
  } else {
    const oppDrewCard = !isFirst && s.opponent && lastState.opponent && s.opponent.handCount > prevOppHandCount;
    lastState = s;
    prevOppDiscardLen = s.opponent?.discard?.length || 0;
    prevOppHandCount  = s.opponent?.handCount || 0;
...
function animateOppRevealAndPlay(playedCard, callback) {
  ...
  if (!existing || !playedCard) { if (existing) zone.innerHTML = ''; callback(); return; }
  ...
  setTimeout(() => { existing.classList.add('face-up'); playCardSound(); }, 280);
  setTimeout(() => { existing.classList.remove('opp-zoom'); existing.classList.add('opp-fly'); }, 700);
  setTimeout(() => { zone.innerHTML = ''; callback(); }, 1250);
}
...
function makeCard(card, faceUp, sizeClass) {
  ...
  const bi = document.createElement('img'); bi.src='assets/cards/back.png'; bi.onerror=()=>bi.style.display='none'; 
...
function openGuessModal(card) {
  ...
  for (let v=0;v<=9;v++) {
    ...
    opt.innerHTML=`
      ...
      ${esc(def.name)}
      <div style="font-size: 9px; color: var(--gold-b); margin-top: 3px; opacity: 0.8;">Видено: ${seen}/${total}</div>
    `;
...
function openZoom(card, seenCounts) {
  ...
  const img=document.createElement('img'); img.src=`assets/cards/${card.value}.png`; img.onerror=()=>img.style.display='none'; 
...
document.getElementById('deck-btn').addEventListener('click', e => {
  ...
  for (let v=0;v<=9;v++) {
    ...
    const img=document.createElement('img'); img.src=`assets/cards/${v}.png`; img.onerror=()=>img.style.display='none'; 
...
function showPeek(data) {
  ...
  document.getElementById('peek-title').textContent=`У ${esc(data.playerName)}`;
